...
class OTPService:
    ...
    def invalidate_old_otps(cls, email):
        OTPRequest.objects.filter(email=email.lower(), is_used=False, expires_at__gt=timezone.now()).update(is_used=True)
    ...
    def send_otp_email(cls, email, otp):
        email = email.lower().strip()
        context = {'otp': otp, 'expiry_minutes': cls.OTP_EXPIRY_MINUTES, 'site_name': getattr(settings, 'SITE_BRAND', 'Plantsindo'), 'support_email': getattr(settings, 'SITE_EMAIL', settings.DEFAULT_FROM_EMAIL)}
        html_message = render_to_string('auth/otp_email.html', context)
        plain_message = render_to_string('auth/otp_email.txt', context)
        try:
            send_mail(subject='Your One-Time Login Code', message=plain_message, from_email=settings.DEFAULT_FROM_EMAIL, recipient_list=[email], html_message=html_message, fail_silently=False)
            return True
    ...
    def verify_otp(cls, email, otp):
        email = email.lower().strip()
        try:
            otp_request = OTPRequest.objects.filter(email=email, is_used=False, expires_at__gt=timezone.now()).latest('created_at')
...
class AuthenticationService:
    ...
    def get_or_create_user(cls, email, name=None):
        ...
        try:
            ...
            user = User.objects.create_user(username=username, email=email, first_name=name or '')
