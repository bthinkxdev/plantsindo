...
from .report_utils import DATE_PRESETS, get_date_range, get_base_order_queryset, apply_report_filters, get_report_summary, report_cache_key, get_orders_report_queryset, get_product_performance_queryset, get_customer_report_queryset, get_inventory_report_queryset, _filter_kwargs_for_report

def _get_report_filters(request):
    get = request.GET
    return {'date_preset': get.get('date_preset') or 'this_month', 'date_from': get.get('date_from'), 'date_to': get.get('date_to'), 'order_status': get.get('status'), 'payment_method': get.get('payment_method'), 'product_id': get.get('product') or get.get('product_id'), 'category_id': get.get('category') or get.get('category_id'), 'user_id': get.get('user') or get.get('user_id'), 'guest_only': get.get('guest_only') == '1' or get.get('guest_only') == 'true', 'min_amount': get.get('min_amount'), 'max_amount': get.get('max_amount'), 'sort': get.get('sort') or '-created_at', 'page': get.get('page', 1), 'page_size': min(int(get.get('page_size') or 20), 100)}
...
class ReportsDashboardView(StaffRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['report_types'] = [{'key': 'sales', 'name': 'Sales Report', 'url': 'admin_panel:report_sales', 'icon': 'fa-chart-line'}]
        return context
...
class OrdersReportView(StaffRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({'active_menu': 'reports', 'report_type': 'orders', 'summary': summary, 'page_obj': page, 'orders': page.object_list, 'date_presets': DATE_PRESETS, 'filters': filters, 'status_choices': Order.Status.choices, 'payment_choices': Payment.Method.choices, 'products': Product.objects.filter(is_active=True).order_by('name')[:200], 'categories': Category.objects.filter(is_active=True).order_by('name'), 'sort': filters['sort']})
        return context
...
class SalesReportView(StaffRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        paginator = Paginator(qs, filters['page_size'])
        page = paginator.get_page(filters['page'])
        context.update({'active_menu': 'reports', 'report_type': 'sales', 'summary': summary, 'page_obj': page, 'orders': page.object_list, 'date_presets': DATE_PRESETS, 'filters': filters, 'status_choices': Order.Status.choices, 'payment_choices': Payment.Method.choices, 'products': Product.objects.filter(is_active=True).order_by('name')[:200], 'categories': Category.objects.filter(is_active=True).order_by('name'), 'sort': filters['sort']})
        return context
...
class ProductPerformanceReportView(StaffRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        paginator = Paginator(qs, filters['page_size'])
        page = paginator.get_page(filters['page'])
        context.update({'active_menu': 'reports', 'report_type': 'products', 'summary': summary, 'page_obj': page, 'products': page.object_list, 'date_presets': DATE_PRESETS, 'filters': filters, 'status_choices': Order.Status.choices, 'payment_choices': Payment.Method.choices, 'categories': Category.objects.filter(is_active=True).order_by('name'), 'filter_products': Product.objects.filter(is_active=True).order_by('name')[:200], 'sort': filters['sort']})
        return context
...
class CustomerReportView(StaffRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        paginator = Paginator(qs, filters['page_size'])
        page = paginator.get_page(filters['page'])
        context.update({'active_menu': 'reports', 'report_type': 'customers', 'summary': summary, 'page_obj': page, 'customers': page.object_list, 'date_presets': DATE_PRESETS, 'filters': filters, 'status_choices': Order.Status.choices, 'payment_choices': Payment.Method.choices, 'sort': filters['sort']})
        return context
...
class InventoryReportView(StaffRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({'active_menu': 'reports', 'report_type': 'inventory', 'variants': variants, 'filters': filters, 'categories': Category.objects.filter(is_active=True).order_by('name'), 'products': Product.objects.filter(is_active=True).order_by('name')[:200], 'summary': {'total_orders': 0, 'total_revenue': Decimal('0'), 'avg_order_value': Decimal('0'), 'top_product_name': '—'}})
        return context
...
class ReportApiView(StaffRequiredMixin, View):
    ...
    def get(self, request, report_type):
        ...
        if report_type in ('orders', 'sales'):
            ...
            paginator = Paginator(qs, filters['page_size'])
            page = paginator.get_page(filters['page'])
            html = render(request, 'admin/reports/partials/orders_table.html', {'orders': page.object_list, 'status_choices': Order.Status.choices}).content.decode('utf-8')
            pagination = {'page': page.number, 'num_pages': paginator.num_pages, 'total_count': paginator.count, 'has_previous': page.has_previous(), 'has_next': page.has_next()}
            ...
            paginator = Paginator(qs, filters['page_size'])
            page = paginator.get_page(filters['page'])
            html = render(request, 'admin/reports/partials/products_table.html', {'products': page.object_list}).content.decode('utf-8')
            pagination = {'page': page.number, 'num_pages': paginator.num_pages, 'total_count': paginator.count, 'has_previous': page.has_previous(), 'has_next': page.has_next()}
            ...
            paginator = Paginator(qs, filters['page_size'])
            page = paginator.get_page(filters['page'])
            html = render(request, 'admin/reports/partials/customers_table.html', {'customers': page.object_list}).content.decode('utf-8')
            pagination = {'page': page.number, 'num_pages': paginator.num_pages, 'total_count': paginator.count, 'has_previous': page.has_previous(), 'has_next': page.has_next()}
        else:
            variants_qs = get_inventory_report_queryset(filters)
            variants = list(variants_qs[:200])
            summary = {'total_orders': 0, 'total_revenue': 0, 'avg_order_value': 0, 'top_product_name': '—'}
            html = render(request, 'admin/reports/partials/inventory_table.html', {'variants': variants}).content.decode('utf-8')
            pagination = {'page': 1, 'num_pages': 1, 'total_count': 0, 'has_previous': False, 'has_next': False}
        if summary:
            summary = {'total_orders': summary['total_orders'], 'total_revenue': str(summary['total_revenue']), 'avg_order_value': str(summary['avg_order_value']), 'top_product_name': summary['top_product_name']}
        ...
        if report_type in ('orders', 'sales'):
            ...
            for o in qs:
                ...
                try:
                    ...
                    pass
                writer.writerow([o.order_number, o.created_at.strftime('%Y-%m-%d %H:%M') if o.created_at else '', o.address.full_name if o.address else '', o.address.phone if o.address else '', o.total, o.get_status_display(), payment_method])
        elif report_type == 'products':
            qs = get_product_performance_queryset(filters)
            qs = _apply_sort_products(qs, filters['sort'])[:10000]
            writer.writerow(['Product', 'Category', 'Units Sold', 'Revenue', 'Orders'])
            for p in qs:
                writer.writerow([p.name, p.category.name if p.category else '', p.units_sold or 0, p.revenue or 0, p.order_count or 0])
        elif report_type == 'customers':
            qs = get_customer_report_queryset(filters)
            qs = _apply_sort_customers(qs, filters['sort'])[:10000]
            writer.writerow(['User', 'Email', 'Orders', 'Total Spent'])
            for u in qs:
                writer.writerow([getattr(u, 'username', ''), getattr(u, 'email', ''), u.order_count or 0, u.total_spent or 0])
        else:
            variants_qs = get_inventory_report_queryset(filters)
        ...
        if report_type in ('orders', 'sales'):
            ...
            for o in qs:
                ...
                try:
                    ...
                    pass
                ws.append([o.order_number, o.created_at.strftime('%Y-%m-%d %H:%M') if o.created_at else '', o.address.full_name if o.address else '', o.address.phone if o.address else '', float(o.total), o.get_status_display(), payment_method])
        elif report_type == 'products':
            qs = get_product_performance_queryset(filters)
            qs = _apply_sort_products(qs, filters['sort'])[:10000]
            ws.append(['Product', 'Category', 'Units Sold', 'Revenue', 'Orders'])
            for p in qs:
                ws.append([p.name, p.category.name if p.category else '', p.units_sold or 0, float(p.revenue or 0), p.order_count or 0])
        elif report_type == 'customers':
            qs = get_customer_report_queryset(filters)
            qs = _apply_sort_customers(qs, filters['sort'])[:10000]
            ws.append(['User', 'Email', 'Orders', 'Total Spent'])
            for u in qs:
                ws.append([getattr(u, 'username', ''), getattr(u, 'email', ''), u.order_count or 0, float(u.total_spent or 0)])
        else:
            variants_qs = get_inventory_report_queryset(filters)
